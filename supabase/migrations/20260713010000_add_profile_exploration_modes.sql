...
add column if not exists exploration_modes text[];
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'profiles_exploration_modes_valid'
      and conrelid = 'public.profiles'::regclass
  ) then
    alter table public.profiles
    add constraint profiles_exploration_modes_valid
    check (
      exploration_modes is null
      or (
        cardinality(exploration_modes) > 0
        and exploration_modes <@ array[
          'solo',
          'couple',
          'family',
          'friends',
        ...
        and array_position(exploration_modes, null) is null
      )
    );
  end if;
end $$;
...
declare
  ...
  exploration_modes_json jsonb := metadata->'exploration_modes';
  exploration_modes text[];
...
begin
  ...
  if is_tourin_registration then
    if exploration_modes_json is null or jsonb_typeof(exploration_modes_json) <> 'array' then
      raise exception 'Missing registration exploration_modes';
    end if;

    select array_agg(selected.mode order by selected.item_index)
    into exploration_modes
    from jsonb_array_elements_text(exploration_modes_json)
      with ordinality as selected(mode, item_index);

    if coalesce(cardinality(exploration_modes), 0) = 0 then
      raise exception 'Missing registration exploration_modes';
    end if;

    if not (
      exploration_modes <@ array[
        'solo',
        'couple',
        'family',
        'friends',
      ...
      raise exception 'Invalid registration exploration_modes';
    end if;

    if cardinality(exploration_modes) <> (
      select count(distinct selected_mode)
      from unnest(exploration_modes) as selected(selected_mode)
    ) then
      raise exception 'Duplicate registration exploration_modes';
    end if;
  ...
  set
    ...
    exploration_modes = coalesce(excluded.exploration_modes, public.profiles.exploration_modes),
